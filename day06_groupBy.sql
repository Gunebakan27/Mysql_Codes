...
==============================================================================*/ 
  ...
  yazınız.
------------------------------------------------------------------------------*/  
    select isim, sum(urun_miktari) toplam_ürün_miktari from manav group by isim ;
   /* ----------------------------------------------------------------------------
...
where urun_adi is not null
group by urun_adi order by max;
...
from manav
group by isim, urun_adi
having sum(urun_miktari)>=3 -- aggregate(sum, avg vs.) kullanildiginda where yerine having kullanilir
