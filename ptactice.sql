...
update people set
name='ad daha sonra eklenecek' where name is null;

-- Boş adresleri 'Adres daha sonra eklenecek' olarak değiştirin
update people set
address='adres daha sonra eklenecek' where address is null;

-- people tablosunda 'Daha sonra eklenecek' tüm boş değerleri değiştirin
update people set
name=coalesce(name, address, 'Daha sonra eklenecek');
...
from (select worker_salary from worker)as maas) * 0.80 
where worker_name='Veli Han';

-- Ali Can'ın maaşını en düşük maaştan yüzde 30 daha fazla düşürun
update worker set
worker_salary=(select min(worker_salary) 
from (select worker_salary from worker)as maas) * 0.70 
where worker_name='ali can';
-- Maaş ortalama maaştan düşükse maaşları 1000 artırın
update worker set
worker_salary<=(select avg(worker_salary)
update worker
set worker_salary = (worker_salary+1000)
where worker_salary < (select avg(worker_salary)
from (select * from worker) as m);
from(select worker_salary from worker)as maas)+1000 where worker_salary<;
...
-- Örnek 7: Her türden kac tane kitap oldugunu listeleyiniz(Tür isimleri de olsun)
select kitap_turu, (select count(turno) 
from kitap where islemler.turno=kitap.turno) as adet_sayisi
from islemler
group by kitap_turu;